...
export type FacilityCategory =
  ...
  | 'cleaning' // 清洁服务
  | 'security'; // 安全与安保
// 自定义分类不在此类型中，直接使用用户输入的名称作为 category
...
export function getCategoryLabel(category: string): string {
  ...
  return preset ? preset.label : category; // 自定义分类直接返回原值
}
...
export interface FrontendFacilityDetail {
  id: string;
  category: string; // 分类代码或自定义分类名
  facilityCode: string; // 设施代码（自定义时为 'custom_xxx'）
  facilityName: string; // 设施显示名称
  description?: string; // 设施描述（可选）
}
